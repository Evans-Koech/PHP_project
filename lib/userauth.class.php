<?php
/**
 * UserAuth class for authentication of users. Needs appconfig.php
 *
 * @author Srinath
 * @version 0.1
	...
	*/
  	...
  	public function logout($reason = 'User Logged out', $die = false) {
		...
		if($this->isLoaded()) {
			...
			unset($_SESSION[SESSION_VARIABLE]);
   			$this->userData = null;
			if($die) {
	  			echo "$reason <br /> Please click <a href='".$this->actualPath."index.php'>here</a> to login again<br />";
				exit;
			}
			else {
	...
	public function insertUser($data) {
		...
		if( $this->db->affected_rows == 1 ) {
			...
			if(AUTO_ACTIVATE) {
				echo "Your account has been created and activated! Click <a href='index.php'>here</a> to login";
				exit;
			}
